 ...
 *
 * ---- 何に使うのか ----
 * 個別の組（1人の講師が同時に見る生徒の集合）を作るときの寄せ方に使う。
 * **講師が担当するのは得意な2科目ほどで、その2つは文系どうし・理系どうしに
 * なりやすい。** 英と国の組は持てる人がいるが、英と数の組は持てる人がまずいない。
 *
 * 実在の講師の担当範囲（TeacherSubject）を見るのがいちばん正しいが、
 * それだけだと**講師の登録が埋まるまで組が作れない。** 系統は塾側にとって
 * 自明な情報なので、入力がほぼ要らないわりに初期状態を救える。
 *
 * どちらとも言えない科目（面談・小論文など塾によって扱いが違うもの）は
 * OTHER にしておく。**OTHER はどちらとも一緒にできる**扱いにする。
 */
export const SUBJECT_STREAM = {
  /** 文系（英語・国語・社会 など） */
...
export const SUBJECT_STREAM_LABEL: Record<string, string> = {
  HUMANITIES: "文系",
  SCIENCE: "理系",
  OTHER: "どちらでも",
};
