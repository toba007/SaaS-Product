 ...
 * 個別の必要人数。
 *
 * 同じ枠・同じ科目に入っている生徒を、まとめられる人数で割る。
 * 「1対1」の生徒だけは1人ずつ数える。
 *
 * 例：上限4人の塾で、英語の火曜2限に 1対1 が1人・その他が5人
 *     → 1 + ceil(5 / 4) = 3人の講師が要る
 */
export function individualDemand(
  ...
  // "date:periodId:subjectId" -> { solo, pooled }
  const acc = new Map<string, { solo: number; pooled: number }>();
  ...
  for (const date of dates) {
    for (const s of schedules) {
      ...
      const key = `${date}:${s.periodId}:${link.subjectId}`;
      const cur = acc.get(key) ?? { solo: 0, pooled: 0 };
      if (indivSizeOf(link.format) === 1) cur.solo++;
      else cur.pooled++;
      acc.set(key, cur);
    }
  }
  ...
  const out: DemandRow[] = [];
  for (const [key, { solo, pooled }] of acc) {
    const [date, periodId, subjectId] = key.split(":");
    const required = solo + Math.ceil(pooled / cap);
...
export function mergeDemand(...groups: DemandRow[][]): DemandRow[] {
  ...
  for (const rows of groups) {
    for (const r of rows) {
      const key = `${r.date}:${r.periodId}:${r.subjectId}:${r.format}`;
